...
function main(array $post): array
{
    ...
    if (!(array_key_exists("auth", $post) && array_key_exists("iconID", $post)))
        throw new MissingParameterException();

    $auth = $connection->realEscapeString($post["auth"]);
    $icon_id = $connection->realEscapeString($post["iconID"]);
    ...
    $result = $connection->select("icon", ["name", "path"], "`id` = '{$icon_id}'");
    if (sizeof($result) === 0)
        throw new ObjectDoesntExistException(); // requested canvas doesn't exist
    $result = $result[0];
    $icon_name = $result["name"];
    $path = $result["path"];
    $icon_img = file_get_contents($path);
    $base64 = base64_encode($icon_img);
    $icon = ["name" => $icon_name, "id" => $icon_id, "img" => $base64];
    $array = ["result" => "ok", "icon" => $icon];
    return $array;
}
