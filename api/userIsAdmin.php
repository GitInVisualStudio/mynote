...
function main(array $post): array
{
    ...
    $result = $connection->select("user_in_course", ["admin"], "`user_id` = '{$user_id_to_get}' AND `course_id` = '{$course_id}'");
    if (sizeof($result) === 0)
        throw new ObjectDoesntExistException(); // user-course pair doesn't exist
    $result = $result[0];
    $admin = $result["admin"];
    $admin = $admin == 1 ? true : false;
    $result = ["result" => "ok", "isAdmin" => $admin];
    return $result;
}
